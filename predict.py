...
def count_vehicle(box_id, img):
    ...
    for item in rois:
        ...
        x1 = int(item[0] * img.shape[1])
        x2 = int(item[2] * img.shape[1])
        y1 = int(item[1] * img.shape[0])
        y2 = int(item[3] * img.shape[0])
        cv2.rectangle(img, (x1, y1), (x2 + x1, y2 + y1), (255,0,0), 2)
        i = item[4]
...
def realTime():
    ...
    blur = cv2.GaussianBlur(heatmapshow,(13,13), 11)
    super_imposed_img = cv2.addWeighted(blur, 0.5, sample_frame, 0.5, 0)
    cv2.imwrite('img.jpg', super_imposed_img)
    cv2.imshow('imddd', super_imposed_img)
    cv2.waitKey(0)
